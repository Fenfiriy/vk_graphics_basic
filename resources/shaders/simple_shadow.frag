...
void main()
{
  ...
  if (Params.vsmEnabled) 
  {
    ...
    float M1 = vsm.x;
    float M2 = vsm.y;
    float diff_r_m1 = posLightSpaceNDC.z - M1;
    float sq_sigma = M2 - M1*M1;
    float value = sq_sigma/(sq_sigma + diff_r_m1*diff_r_m1);
    shadow = ((posLightSpaceNDC.z < M1 + 0.001f) || outOfView) ? 1.0f : value;
  }
  else 
  {
